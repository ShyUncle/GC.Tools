...
namespace MachineLearning
{
    public class Rec
    {
        ...
        public Rec(  OcrVersionConfig config)
        {
            var sessionOptions = new SessionOptions();
            _config = config;
            _session = new InferenceSession("PaddleOCR/ch_PP-OCRv4_rec/slim_model.onnx", sessionOptions); 
            _labels = File.ReadAllLines("PaddleOCR/ppocr_keys_v1.txt"); // File.ReadAllLines("PaddleOCR/chinese_cht_dict.txt");
        }
        ...
        public string[] Run(Mat[] srcs, int batchSize = 0)
        {
        ...
        public string Run(Mat src)
        {
            return RunMulti([src]).Single();
        }

        private string[] RunMulti(Mat[] srcs)
        {
            if (srcs.Length == 0) return [];
