...
namespace DnsHiJacking
{
    internal class DnsInterceptor
    {
        ...
        unsafe private void ModifyDnsPacket(WinDivertBuffer winDivertBuffer, ref WinDivertAddress winDivertAddress, ref uint packetLength)
        {
            ...
            if (new Domain("www.baidu.com").ToString() != question.Name.ToString() && new Domain("a.fastgithub.com").ToString() != question.Name.ToString()) //  if (this.fastGithubConfig.IsMatch(question.Name.ToString()) == false)
            {
                return;
            }
            ...
            Console.WriteLine($"{domain}->{loopback}");
        }
